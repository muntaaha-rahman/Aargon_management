...
class Service(Base):
    ...
    created_by = Column(String(100), nullable=False)  # String, not ForeignKey
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
    
    # Back relationship to service assignments
...
class ServiceAssignment(Base):
    ...
    # Individual flexible fields - each entry will have these 3 fields
